...
function menu(F, I) {
    ...
    this.onmouseover = W;
    this.onmousedown = U;
    this.J = {'block_top':16,'block_left':16,'top':72,'left':4,'width':120,'height':22,'hide_delay':0,'expd_delay':0,'css':{'inner':'','outer':''}};
    this.O = function(p) {
        return this.J[p];
    };
    this.n = this;
...
function T(b) {
    ...
    if (l.B[2] && l.B[2]['tw'])window.open(q, l.B[2]['tw']); else window.location = q;
    return true;
}
function V(b) {
    var l = this.D[b];
...
function S(m, c) {
    ...
    this.A = [];
    for (var c = 0; c < this.B.length - 3; c++)new S(this, c);
}
function X(p) {
