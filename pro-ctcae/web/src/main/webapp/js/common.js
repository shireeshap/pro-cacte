...
Object.extend(ListEditor.prototype, {
    ...
    add: function() {
        ...
        if (!addFn) {
            ...
            if (!parameterizedFn) {
                alert("There is no function the selected dwr namespace named either " + specificFnName + " or " + parameterizedFnName);
                return;
            }
    ...
    up: function(event) {
        this._reorderEventHandler(event, function(i) {
            return +i - 1
        })
    },
    ...
    remove: function(evt) {
        ...
        delArgs = delArgs.concat([function(ajaxOutput) {
            ...
            $$("div." + this.divisionClass + " .list-controls").each(function(e) {
                e.conceal();
            })
