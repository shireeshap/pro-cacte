...
public class ClinicalStaffQueryTest extends TestCase {
    ...
    public void testQueryConstructor() throws Exception {
        ...
        assertEquals("SELECT cs from ClinicalStaff cs WHERE ((cs.status =:status1 and cs.effectiveDate <=:effectiveDate) or (cs.status =:status2 and cs.effectiveDate > :effectiveDate)) AND lower(cs.firstName) LIKE :firstName order by cs.id",
    ...
    public void testFilterByOrganizationId() throws Exception {
        ...
        assertEquals("SELECT cs from ClinicalStaff cs left join cs.organizationClinicalStaffs as scs WHERE ((cs.status =:status1 and cs.effectiveDate <=:effectiveDate) or (cs.status =:status2 and cs.effectiveDate > :effectiveDate)) AND scs.organization.id = :organizationId order by cs.id",
                clinicalStaffQuery.getQueryString());
        assertEquals("wrong number of parameters", clinicalStaffQuery.getParameterMap().size(), 4);
    ...
    public void testFilterByNciIdentifier() throws Exception {
        ...
        assertEquals("SELECT cs from ClinicalStaff cs WHERE ((cs.status =:status1 and cs.effectiveDate <=:effectiveDate) or (cs.status =:status2 and cs.effectiveDate > :effectiveDate)) AND lower(cs.nciIdentifier) LIKE :nciIdentifier order by cs.id",
                clinicalStaffQuery.getQueryString());
        assertEquals("wrong number of parameters", clinicalStaffQuery.getParameterMap().size(), 4);
    ...
    public void testFilterByFullName() throws Exception {
        ...
        clinicalStaffQuery.filterByClinicalStaffFirstName("John");
        clinicalStaffQuery.filterByClinicalStaffLastName("Dow");
        assertEquals("SELECT cs from ClinicalStaff cs WHERE ((cs.status =:status1 and cs.effectiveDate <=:effectiveDate) or (cs.status =:status2 and cs.effectiveDate > :effectiveDate)) AND lower(cs.lastName) LIKE :lastName AND lower(cs.firstName) LIKE :firstName order by cs.id",
                clinicalStaffQuery.getQueryString());
