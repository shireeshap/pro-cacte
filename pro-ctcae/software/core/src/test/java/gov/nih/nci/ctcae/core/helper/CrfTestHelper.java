...
//        }
   ...
   private static void setProCtcTermCurrencyForAllTerms(){
    ...
    /* Create testForms for each proCtcTerm present in the Db. 
	 * e.g. create 80 CRFs such that each CRF only has questions corresponding to one proCtcTerm from db.
	 */
    public static List<Integer> createAccruFormsForEachProCtcTerm() throws Exception {
        ...
        for(ProCtcTerm proCtcTerm : proCtcTerms){
        	CRF crf = new CRF();
        	crf.setTitle(formTitle + "_" + proCtcTerm.getTerm());
        	firstTab_SelectStudy(crf, StudyTestHelper.getDefaultStudy());
        	crf.setCrfVersion("1.0");
            crf.addProCtcTerm(proCtcTerm);
            crf.updateCrfPageInstructions();
            crf = crfRepository.save(crf);
            accruCrfIds.add(crf.getId());
            
            thirdTab_ScheduleTemplateForACCRUForms(crf, proCtcTerm);
            //thirdTab_ScheduleTemplate(crf);
            crfRepository.save(crf);
            fourthTab_Notifications(crf);
            crfRepository.save(crf);
    ...
    private static void thirdTab_ScheduleTemplateForACCRUForms(CRF crf, ProCtcTerm proCtcTerm) {
        for (FormArmSchedule formArmSchedule : crf.getFormArmSchedules()) {
            CRFCalendar calendar = new CRFCalendar();
            calendar.setRepeatEveryValue("1");
            calendar.setRepeatEveryUnit("Days");
            calendar.setDueDateValue("24");
            calendar.setDueDateUnit("Hours");
            calendar.setRepeatUntilValue("1");
            calendar.setRepeatUntilUnit("Number");

            formArmSchedule.addCrfCalendar(calendar);

            CRFCycleDefinition defA = new CRFCycleDefinition();
            defA.setCycleLength(proCtcTerm.getProCtcGradeMappings().size());
            defA.setCycleLengthUnit("Days");
            defA.setRepeatTimes("1");
            defA.setOrder(0);

            CRFCycle cycle1 = new CRFCycle();
            cycle1.setOrder(0);
            cycle1.setCycleDays(getCycyleDaysOfLength(proCtcTerm.getProCtcGradeMappings().size()));
            defA.addCrfCycle(cycle1);
    ...
    public static String getCycyleDaysOfLength(int length){
    	String cycleDays = "";
