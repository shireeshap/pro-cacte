...
<tr id="row-${index}" style="height:30px;">
    ...
    <c:choose>
    	<c:when test="${readOnly}">
		    ...
		    <td width="25%" ><b>${leadCRA.roleStatus.displayName}</b> <br/>since <tags:formatDate value="${leadCRA.statusDate}"/>
		    </td>
    	</c:when>
    </c:choose>
    <c:choose>
    	<c:when test="${not readOnly}">
		    <td width="5%">
				     <a id="del-${empty idSuffix ? index : idSuffix}" class="del-${cssClass}" href="javascript:deleteLeadCra('${index}', '${param.studyId}');">
