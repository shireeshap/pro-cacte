...
<div id="debug">
    ...
    <dl>
        <c:forEach items="<%= new java.util.TreeMap(request.getParameterMap()) %>" var="item">
            ...
            <dd>${commons:nl2br(fn:join(item.value, ', '))}</dd>
        </c:forEach>
    </dl>
    ...
    <dl>
        <c:forEach items="${cookie}" var="item">
            <dt class="var">${item.key}</dt>
            <dd>${commons:nl2br(item.value.value)}</dd>
        </c:forEach>
    </dl>
    ...
    <dl>
        <c:forEach items="<%= edu.nwu.bioinformatics.commons.WebUtils.requestAttributesToMap(request) %>" var="item">
            <dt class="var">${item.key}</dt>
            <dd>${commons:nl2br(item.value)} <em class="clazz">${commons:classname(item.value)}</em></dd>
        </c:forEach>
    </dl>
    ...
    <dl>
        <c:forEach
                items="<%= edu.nwu.bioinformatics.commons.WebUtils.sessionAttributesToMap(request.getSession(false)) %>"
                var="item">
            <dt class="var">${item.key}</dt>
            <dd>${commons:nl2br(item.value)} <em class="clazz">${commons:classname(item.value)}</em></dd>
        </c:forEach>
    ...
    <dl>
        <c:forEach items="<%= edu.nwu.bioinformatics.commons.WebUtils.servletContextAttributesToMap(application) %>"
                   var="item">
            <dt class="var">${item.key}</dt>
            <dd>${commons:nl2br(item.value)} <em class="clazz">${commons:classname(item.value)}</em></dd>
        </c:forEach>
