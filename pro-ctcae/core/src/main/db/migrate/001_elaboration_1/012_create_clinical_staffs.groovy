...
class CreateClinicalStaffs extends edu.northwestern.bioinformatics.bering.Migration {
  void up() {
    ...
    //Cannot have identifiers longer than 30 chars in oracle
    createTable("STUDY_ORG_CLINICAL_STAFFS") {t ->
      t.addVersionColumn()
    ...
    execute('ALTER TABLE STUDY_ORG_CLINICAL_STAFFS ADD CONSTRAINT fk_ss_cls_site_clinical_staff FOREIGN KEY (organization_clinical_staff_id) REFERENCES ORGANIZATION_CLINICAL_STAFFS')
    execute('ALTER TABLE STUDY_ORG_CLINICAL_STAFFS ADD CONSTRAINT fk_ss_cls_study_site FOREIGN KEY (study_organization_id) REFERENCES STUDY_ORGANIZATIONS')

    //Cannot have identifiers longer than 30 chars in oracle
    createTable("STUDY_PAR_CLINICAL_STAFFS") {t ->
      t.addVersionColumn()
      ...
      t.addColumn('notify', 'boolean', nullable: true)
    }


    execute('ALTER TABLE STUDY_PAR_CLINICAL_STAFFS ADD CONSTRAINT fk_sp_cls_so_clinical_staff  FOREIGN KEY (so_clinical_staff_id) REFERENCES STUDY_ORG_CLINICAL_STAFFS')
    execute('ALTER TABLE STUDY_PAR_CLINICAL_STAFFS ADD CONSTRAINT  fk_sp_cls_site_clinical_staff FOREIGN KEY (sp_assignment_id) REFERENCES STUDY_PARTICIPANT_ASSIGNMENTS')
