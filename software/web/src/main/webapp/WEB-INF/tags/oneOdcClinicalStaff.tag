...
<tr id="row-${index}" style="height:30px;">
    ...
    <c:choose>
    	<c:when test="${readOnly}">
		    ...
		    <td width="25%" ><b>${odc.roleStatus.displayName}</b> since <br/><tags:formatDate value="${odc.statusDate}"/>
		    </td>
    	</c:when>
    </c:choose>
     <c:choose>
    	<c:when test="${not readOnly}">
		    <td width="5%">
				     <a id="del-${empty idSuffix ? index : idSuffix}" class="del-${cssClass}" href="javascript:deleteODC('${index}', '${param.studyId}');">
